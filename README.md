...
## Centralisation des logs avec Docker

La version 3.0 peut envoyer les logs journaliers vers un service Docker centralise.  
Ce service recoit les entrees HTTP depuis EasySave et les ajoute dans un fichier quotidien unique au format **NDJSON** :
...
- `serverTimestamp` : date de reception cote serveur
- `clientId` : identifiant du poste/client EasySave
- `format` : format configure cote application (`Json` ou `Xml`)
- `entry` : entree de log EasyLog originale

### Construire l'image

Depuis le dossier `LogCentralizer` :
...
Si Docker ne peut pas contacter `mcr.microsoft.com`, verifier la connexion reseau/DNS de Docker Desktop puis relancer la commande.
...
```json
{"service":"EasySave Log Centralizer","status":"ok","storage":"/data"}
```

### Configurer EasySave GUI

Dans l'interface graphique, aller dans les parametres de logs :

| Parametre | Valeur de demonstration |
|-----------|--------------------------|
| Destination des logs | `Central` ou `Both` |
| Endpoint central | `http://localhost:8080/api/logs` |
| Client id | Nom du poste ou identifiant demo, par exemple `SANDEVISTAN` |

Le mode `Both` est recommande en demonstration : l'application conserve un log local et envoie aussi une copie au serveur central.

### Observer le fichier centralise

Lister les fichiers crees dans le volume Docker :

```powershell
docker exec easysave-log-centralizer ls /data
```

Lire le fichier du jour :

```powershell
docker exec easysave-log-centralizer cat /data/2026-05-09.ndjson
```

Avec Git Bash, ajouter `MSYS_NO_PATHCONV=1` pour eviter que `/data/...` soit transforme en chemin Windows :

```bash
MSYS_NO_PATHCONV=1 docker exec easysave-log-centralizer cat /data/2026-05-09.ndjson
```

Le fichier est aussi visible dans Docker Desktop :

```text
Docker Desktop -> Volumes -> easysave_logs
```
...
|----------|----------------|------------|
| `curl http://localhost:8080/` ne repond pas | Conteneur arrete ou port non publie | `docker ps`, puis relancer avec `-p 8080:8080` |
| Test central logging retourne `500` | Erreur serveur dans `LogCentralizer` | Lire `docker logs easysave-log-centralizer` |
| Erreur `The node already has a parent` | Une entree JSON recue etait reutilisee sans clonage | Corrige dans `LogCentralizer/Program.cs` avec `entry.DeepClone()` |
| `cat: C:/Program Files/Git/data/...` | Conversion automatique des chemins par Git Bash | Utiliser PowerShell ou `MSYS_NO_PATHCONV=1` |
| Aucun fichier `.ndjson` | Aucune entree recue ou mauvais endpoint | Verifier `http://localhost:8080/api/logs` dans les parametres GUI |

Apres une modification du code du centralizer, reconstruire l'image puis remplacer le conteneur :

```powershell
docker build -t easysave-log-centralizer .
docker rm -f easysave-log-centralizer
docker run -d --name easysave-log-centralizer -p 8080:8080 -v easysave_logs:/data easysave-log-centralizer
```
